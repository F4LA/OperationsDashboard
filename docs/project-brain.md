...
_Last updated: 2026-08-19 — Bernardo (cadena de Sprint Planning repartida por D-100; convención de prefijo por D-099; §2 y §8 documentados y §1 completado, spec en v2.2, sin cambios de código)_

> **Source of truth for WHAT to build** is the spec: `docs/spec.md` (v2.2).
...
- Engineering spec written and approved — `docs/spec.md` (v1.1 en su momento, hoy en v2.2; hand-reformatted from the Word original; content complete, worth a diff against the original if byte-exactness ever matters).
...
- Reapertura de la Fase 8 parte 2B (75a925a y a18e536): la semana abre vacía con panel de disponibles, selector Closed/Current/Next week con etiqueta y ventana, el modo lo decide confirmWeek y no el día ni la posición — lo que corrigió un error de una semana en el paso 8 que habría aparecido en el primer L10 real — quitar restituido como deshacer de la propia adición, ad-hoc limitado a semanas no cerradas, y el campo deadline de milestone (11b1d45). Versión en 20260816c. Decisiones: D-087, D-090 a D-095.
- Fase 9 — §13 Issues (a75b52e, 691 tests): pestaña Issues, RPC createIssue, par resolveIssue/unresolveIssue con resolución obligatoria validada en el servidor, validación de sourceIssueId contra un issue real, lector en el cliente, dashboard/issues.js con su tercer segmento, y el seguimiento del §13.4. Verificado de punta a punta en producción el 2026-08-18 (I-0001 escrito con sprintId, raisedBy y raisedAt del servidor). Web App redesplegado a mano en Version 4. Dos bugs encontrados en producción y cerrados el mismo día: contraste (77e078a, D-097) y doble escritura por listener duplicado (ef72e33, D-098, 704 tests). Versión de assets en 20260816f. Decisiones: D-096, D-097, D-098.
...
├── dashboard/  validate.js engine.js events.js board.js metrics.js thisweek.js todos.js issues.js (real; network.js borrado, D-082b)
...
│          issues.test.js   (real, D-096)
│          listener-isolation.test.js   (real, D-098)
├── data/rock3-seed.json   (real, test fixture, per D-016)
└── docs/  spec.md (real, v2.2) · decision-log.md · project-brain.md
```
...
## Next up (per spec §9)
- Carga de R1 y R2 en sprint-plan.json. ÚNICO bloqueante real del dashboard. Esperando de Miguel y Emery los workDays/waitDays por tarea; los borradores estructurales ya están hechos.
...
- El skill web-design-guidelines audita el markup nuevo, no la combinación elemento × fondo. Falló en detectar dos botones ilegibles (D-097). Tenerlo en cuenta: una revisión automática de accesibilidad no reemplaza mirar la pantalla renderizada.
